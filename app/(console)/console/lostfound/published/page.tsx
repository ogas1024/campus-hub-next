...
function buildHref(params: { q?: string; type?: string; page?: number; pageSize?: number }) {
  ...
  return qs ? `/console/lostfound/published?${qs}` : "/console/lostfound/published";
}

export default async function ConsoleLostfoundPublishedPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-4">
          <form className="grid gap-3 md:grid-cols-12" action="/console/lostfound/published" method="GET">
            ...
            <div className="flex flex-wrap gap-2 md:col-span-12">
              <Link className={buttonVariants({ variant: "outline", size: "sm" })} href="/console/lostfound/published">
                清空
              </Link>
            ...
            {data.items.map((item) => (
              <tr key={item.id} className="border-t border-border">
                <td className="px-3 py-3">
                  ...
                  {item.solvedAt ? <Badge variant="secondary" className="mt-1">已解决</Badge> : null}
                </td>
                <td className="px-3 py-3">
                ...
                <td className="px-3 py-3 text-xs text-muted-foreground">{formatZhDateTime(item.publishedAt)}</td>
                <td className="px-3 py-3 text-right">
                  <ConsoleLostfoundActions
                    ...
                    canDelete={canDelete}
                    compact
                    afterDeleteHref="/console/lostfound/published"
                  />
                </td>
              </tr>
            ))}
