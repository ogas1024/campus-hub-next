...
export default function ConsoleSurveyResultsPage({ params }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        const res = await fetchConsoleSurveyResults(params.id);
        if (cancelled) return;
        setData(res);
      } catch (err) {
        if (cancelled) return;
  ...
  async function doAiSummary() {
    ...
    setAiPending(true);
    try {
      const res = await fetchConsoleSurveyAiSummary(params.id);
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link className={buttonVariants({ variant: "outline", size: "sm" })} href={`/console/surveys/${params.id}/edit`}>
            编辑
          </Link>
          <a className={buttonVariants({ variant: "outline", size: "sm" })} href={buildConsoleSurveyExportUrl(params.id)}>
            导出 CSV
          </a>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="space-y-3 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Button
              size="sm"
              disabled={aiPending || effectiveStatus !== "closed"}
              onClick={() => void doAiSummary()}
              title={effectiveStatus !== "closed" ? "问卷结束后才允许生成 AI 总结" : undefined}
            >
