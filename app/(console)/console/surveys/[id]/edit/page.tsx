...
export default function ConsoleSurveyEditPage({ params }: Props) {
    ...
    void (async () => {
      ...
      try {
        const [detail, options] = await Promise.all([
          fetchConsoleSurveyDetail(params.id),
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const res = await action.run(() => updateConsoleSurveyDraft(params.id, body), { fallbackErrorMessage: "保存失败" });
      if (!res) return;
  ...
  async function doPublish() {
    const res = await action.run(() => publishConsoleSurvey(params.id), { fallbackErrorMessage: "发布失败" });
    if (!res) return;
    setStatus(res.status);
    setEffectiveStatus(res.effectiveStatus);
    router.refresh();
  ...
  async function doClose() {
    const res = await action.run(() => closeConsoleSurvey(params.id), { fallbackErrorMessage: "关闭失败" });
    if (!res) return;
    setStatus(res.status);
    setEffectiveStatus(res.effectiveStatus);
    router.refresh();
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link className={buttonVariants({ variant: "outline", size: "sm" })} href={`/console/surveys/${params.id}/results`}>
            查看结果
          </Link>
