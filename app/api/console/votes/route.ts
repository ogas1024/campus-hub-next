...
import { createVoteDraftBodySchema } from "@/lib/modules/votes/votes.schemas";
import { createVoteDraft, listConsoleVotes } from "@/lib/modules/votes/votes.service";
...
export async function POST(request: Request) {
  try {
    const user = await requirePerm("campus:vote:create");
    ...
    const parsed = createVoteDraftBodySchema.safeParse(body);
    if (!parsed.success) throw badRequest("参数校验失败", parsed.error.flatten());

    const created = await createVoteDraft({
      actorUserId: user.id,
      body: {
