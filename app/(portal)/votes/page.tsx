...
function buildPortalVotesHref(params: { q?: string; page?: number; pageSize?: number }) {
  const sp = new URLSearchParams();
  if (params.q && params.q.trim()) sp.set("q", params.q.trim());
  if (params.page && params.page > 1) sp.set("page", String(params.page));
  if (params.pageSize && params.pageSize !== 20) sp.set("pageSize", String(params.pageSize));
  const query = sp.toString();
  return query ? `/votes?${query}` : "/votes";
}

export default async function VotesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const data = await listPortalVotes({
    userId: user.id,
    page,
    pageSize,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {data.items.length === 0 ? (
          <Card>
            <CardContent className="p-10 text-center text-sm text-muted-foreground">暂无可参与投票</CardContent>
          </Card>
        ) : null}
            ...
            <Card className={cn("hover:bg-accent", item.phase === "active" ? "border-primary" : null)}>
              <CardContent className="space-y-2 p-5">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {item.submittedAt ? <Badge variant="outline">已投</Badge> : <Badge variant="secondary">未投</Badge>}
                </div>
