...
export default async function MyVotesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const baseHref = buildMyVotesHref({ q, page: displayPage, pageSize });

  return (
    <div className="space-y-4">
      <PageHeader
        title="我的投票"
        description="仅展示我已参与的投票（包含归档）。"
        meta={
          <>
      ...
      <div className="space-y-3">
        ...
        {data.items.map((item) => (
          ...
          >
            <Card className={cn("transition-colors duration-[var(--motion-duration-hover)] ease-[var(--motion-ease-standard)] hover:bg-accent", item.archivedAt ? "border-muted" : item.phase === "active" ? "border-primary" : null)}>
